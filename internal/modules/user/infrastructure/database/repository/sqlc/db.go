...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.getUserByEmailAndProviderStmt, err = db.PrepareContext(ctx, getUserByEmailAndProvider); err != nil {
		return nil, fmt.Errorf("error preparing query GetUserByEmailAndProvider: %w", err)
	}
...
func (q *Queries) Close() error {
	...
	if q.getUserByEmailAndProviderStmt != nil {
		if cerr := q.getUserByEmailAndProviderStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing getUserByEmailAndProviderStmt: %w", cerr)
		}
	}
...
type Queries struct {
	...
	getUserByEmailAndProviderStmt *sql.Stmt
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		...
		getUserByEmailAndProviderStmt: q.getUserByEmailAndProviderStmt,
